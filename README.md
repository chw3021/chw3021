# 👋 안녕하세요! 차현욱입니다.

👨‍💻 저는 다양한 기술을 활용하여 웹 애플리케이션과 솔루션 개발에 열정을 가진 개발자입니다. 빠른 적응력과 뛰어난 협업 능력을 바탕으로, 팀과 함께 최적의 결과를 만들어내는 것을 목표로 하고 있습니다.

## 📚 교육
- **KH정보교육원**  
  (디지털컨버전스)공공데이터 융합 자바개발자 양성과정A30(20)_1회차
  2024. 09. 26 ~ 2025. 03. 14
- **KH정보교육원**  
  Python 기반의 AI 입문 과정  
  2024. 10. 17 ~ 2024. 11. 13

## 💻 기술 스택

| 프로그래밍 언어 | 프레임워크 | 데이터베이스 | 도구 | 기타 |
|----------------|------------|--------------|------|------|
| ![Java](https://img.icons8.com/color/48/000000/java-coffee-cup-logo.png) Java  | ![Spring Boot](https://img.icons8.com/color/48/000000/spring-logo.png) Spring Boot | ![MySQL](https://img.icons8.com/color/48/000000/mysql-logo.png) MySQL | ![VS Code](https://img.icons8.com/color/48/000000/visual-studio-code-2019.png) Visual Studio Code | ![GitHub](https://img.icons8.com/color/48/000000/github-2.png) GitHub |
| ![JavaScript](https://img.icons8.com/color/48/000000/javascript.png) JavaScript | ![Flask](https://img.icons8.com/color/48/000000/flask.png) Flask | ![MariaDB](https://img.icons8.com/?size=48&id=nrY6pkbRkJCi&format=png&color=000000) MariaDB | ![Eclipse](https://img.icons8.com/color/48/000000/eclipse.png) Eclipse | ![Firebase](https://img.icons8.com/color/48/000000/firebase.png) Firebase |
| ![Python](https://img.icons8.com/color/48/000000/python.png) Python | ![React](https://img.icons8.com/color/48/000000/react-native.png) React | ![Oracle](https://img.icons8.com/color/48/000000/oracle-logo.png) Oracle | ![Android Studio](https://img.icons8.com/color/48/000000/android-studio.png) Android Studio | ![AWS](https://img.icons8.com/color/48/000000/amazon-web-services.png) AWS |



## 🌟 프로젝트 경험

### 1. [뮤직커버리(Musicovery)](https://github.com/chw3021/MusicoveryReact.git)
- **수행기간**: 2025.02.13 ~ 2025.03.13
- **개발목표**: 사용자의 취향에 맞는 음악 추천 및 소셜 상호작용 제공
- **주요기능**:
  - Spotify API를 활용한 음악 추천
  - 사용자 맞춤형 플레이리스트 생성
  - 소셜 기능 (좋아요, 댓글, 친구 추가)

---

### 2. [CompanyDefense](https://github.com/chw3021/CompanyDefense.git)
- **수행기간**: 2025.01.14 ~ 2025.03.06
- **개발목표**: 모바일 환경에서 즐길 수 있는 랜덤 타워 디펜스 게임 개발
- **주요기능**:
  - 다양한 타워와 적군 시스템
  - Firebase를 통한 데이터 저장 및 동기화
  - 사용자 로그인 및 UI/UX 개선

---

### 3. [RpsSkills plugin for Minecraft](https://github.com/chw3021/rpgskills.github.io.git)
- **수행기간**: 2020.01.05 ~ 2025.01.26
- **개발목표**: Minecraft 서버를 RPG 게임처럼 변환하는 플러그인 개발
- **주요기능**:
  - 커스텀 몬스터, 무기, 직업, 스킬, 퀘스트 시스템
  - 바이옴 기반 몬스터 스케일링 및 무기 강화
- **다운로드 링크**:
https://www.spigotmc.org/threads/rpgskills.676660/

---

### 4. [북마카세(Bookmakase)](https://github.com/chw3021/bookmakase.git)
- **수행기간**: 2023.04.17 ~ 2023.05.31
- **개발목표**: 사용자 맞춤형 도서 추천 서비스 플랫폼 구축
- **주요기능**:
  - Interpark Book API를 활용한 도서 추천 기능
  - 독서 목표 및 일지 시스템
  - 사용자 리뷰 및 커뮤니티 기능

---

## 🌱 앞으로의 목표
- 지속적인 기술 학습과 실무 경험을 통해, 대규모 트래픽을 처리하는 웹 애플리케이션 및 엔터프라이즈 솔루션 개발에 필요한 기술을 심화 학습하고, 실질적인 성과를 창출할 수 있는 개발자로 성장하고자 합니다.

## 📫 연락처
- 이메일: [your_email@example.com](mailto:chw3021@naver.com)
- GitHub: [chw3021](https://github.com/chw3021)

감사합니다! 😊
